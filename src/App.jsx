    ...
    const tryAsk = async (attempt) => {
      try {
        ...
        if (attempt < 2) {
          ...
          setMessages(prev => [...prev, { role: 'assistant', content: '⚠️ Could not connect to AI. Please check your internet and try again.' }])
        }
      }
    }
    await tryAsk(0)
  ...
  return (
    <div style={{ width: '100%', maxWidth: 680, flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px 12px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <button onClick={onEnd} style={{ ...S.backBtn, marginBottom: 0 }}>← Change Subject</button>
      </div>
