...
// Plans — maxUses is now UNLIMITED per device; access is TIME-BASED only
const PLANS = [
  { id: 'session',  label: 'Single Session',  icon: '⚡',  price: 500,   duration: 24*60*60*1000,    color: '#4CAF50', badge: null,         desc: '24 hours unlimited access',          detail: 'Come back as many times as you want for 24 hours.',                           prefix: 'EDU', maxDevices: 3   },
  { id: 'weekly',   label: 'Weekly Plan',     icon: '📅',  price: 2500,  duration: 7*24*60*60*1000,  color: '#2196F3', badge: 'POPULAR',     desc: '7 days unlimited access',            detail: 'Unlimited sessions for a full week. Study any subject, any time.',            prefix: 'WEK', maxDevices: 3   },
  { id: 'monthly',  label: 'Monthly Plan',    icon: '🗓️', price: 5000,  duration: 30*24*60*60*1000, color: '#FF9800', badge: 'BEST VALUE',  desc: '30 days unlimited access',           detail: 'One full month of unlimited AI tutoring across all subjects.',                prefix: 'MON', maxDevices: 3   },
  { id: 'school',   label: 'School Package',  icon: '🏫',  price: 50000, duration: 30*24*60*60*1000, color: '#9C27B0', badge: 'FOR SCHOOLS', desc: 'Unlimited students · 30 days',       detail: 'One code shared with your entire school. Unlimited students, unlimited use.',  prefix: 'SCH', maxDevices: 9999 },
]
...
function getPlanFromCode(code) {
  ...
  return PLANS.find(p => p.prefix === prefix) || null
}
    ...
    <div style={S.centered}>
      ...
      <div style={{ width: '100%', marginTop: 8, background: 'rgba(37,211,102,0.07)', border: '1px solid rgba(37,211,102,0.25)', borderRadius: 14, padding: '14px 16px' }}>
        <div style={{ fontSize: 12, color: '#888', textAlign: 'center', marginBottom: 10 }}>
          💬 Prefer to pay manually via MoMo?
        </div>
        <a
        ...
        >
          ...
          Contact EduCam on WhatsApp
        </a>
        <div style={{ fontSize: 11, color: '#666', textAlign: 'center', marginTop: 8 }}>
          Send money via MTN/Orange MoMo → Get your code instantly
        </div>
      </div>
...
function PlanScreen({ onSelect, onBack }) {
  return (
    <div style={{ ...S.centered, maxWidth: 560 }}>
      ...
      <p style={{ color: '#888', fontSize: 14, textAlign: 'center', marginBottom: 24 }}>All plans give unlimited access until they expire.</p>
      ...
      <p style={{ color: '#555', fontSize: 11, textAlign: 'center', marginTop: 6 }}>Send MoMo to EduCam → Receive your access code on WhatsApp</p>
    </div>
  )
}
...
function PaymentScreen({ plan, onSuccess, onBack }) {
  ...
  if (step === 'success') return (
    <div style={S.centered}>
      ...
      <button onClick={() => onSuccess(generatedCode, plan.id)} style={{ ...S.btnPrimary, background: 'linear-gradient(135deg,' + plan.color + '99,' + plan.color + ')' }}>🚀 Start Studying Now</button>
    </div>
  )
...
function CodeEntryScreen({ onSuccess, onBack }) {
  ...
  const previewPlan = val => {
    ...
    if (val.length >= 3) {
      const plan = getPlanFromCode(val.toUpperCase())
  ...
  return (
    <div style={S.centered}>
      ...
      <p style={{ color: '#888', textAlign: 'center', marginBottom: 8, fontSize: 13 }}>Enter the code from your payment, or the code shared by your school.</p>
      <div style={S.infoBox}>✅ Your code gives <strong>unlimited access</strong> until it expires — go in and out as many times as you want!</div>
      ...
      <div style={{ width: '100%', marginTop: 16, borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 16 }}>
        <p style={{ color: '#666', fontSize: 12, textAlign: 'center', marginBottom: 10 }}>Or contact us directly to pay manually:</p>
        <a
        ...
        >
          ...
          WhatsApp EduCam Team
        </a>
        <p style={{ color: '#555', fontSize: 11, textAlign: 'center', marginTop: 6 }}>+237 682 613 235 · We reply fast! 🇨🇲</p>
      </div>
    </div>
  )
}
...
function SelectorScreen({ planId, onStart, onBack }) {
  ...
  return (
    <div style={{ width: '100%', maxWidth: 680, padding: '20px 24px 40px' }}>
      ...
      <p style={{ color: '#888', fontSize: 13, marginBottom: 20 }}>You can switch subjects and come back anytime. Choose your exam and subject to begin.</p>
...
function ChatScreen({ subject, exam, planId, onEnd }) {
  ...
  const welcomeMsg = { role: 'assistant', content: '🎓 Welcome! I am your EduCam AI tutor for **' + (subj?.label || '') + '** — ' + exam + ' level.\n\nAsk me anything in English or French. Let us pass this exam together! 🇨🇲' }
    ...
    const tryAsk = async (attempt) => {
      try {
        ...
        const reply = await apiChat(
          apiMsgs,
          SYSTEM_PROMPT + ' Subject: ' + (subj?.label || '') + '. Exam: ' + exam + '.'
        )
    ...
    <div style={{ width: '100%', maxWidth: 680, flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px 10px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          {isReturning && !showClearConfirm && (
            <button onClick={() => setShowClearConfirm(true)} style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.25)', borderRadius: 16, padding: '4px 10px', color: '#f87171', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>🗑️ Clear</button>
          )}
          {showClearConfirm && (
            <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
              <span style={{ fontSize: 11, color: '#f87171' }}>Clear all history?</span>
              <button onClick={handleClearHistory} style={{ background: '#f87171', border: 'none', borderRadius: 12, padding: '3px 10px', color: '#fff', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>Yes</button>
              <button onClick={() => setShowClearConfirm(false)} style={{ background: 'rgba(255,255,255,0.08)', border: 'none', borderRadius: 12, padding: '3px 10px', color: '#aaa', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>No</button>
            </div>
          )}
          <button onClick={onEnd} style={{ ...S.backBtn, marginBottom: 0 }}>← Subjects</button>
        </div>
      </div>
      ...
      {isReturning && (
        <div style={{ margin: '8px 20px 0', background: 'rgba(76,175,80,0.08)', border: '1px solid rgba(76,175,80,0.2)', borderRadius: 8, padding: '7px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div>
            <span style={{ fontSize: 12, color: '#4CAF50', fontWeight: 'bold' }}>Welcome back! </span>
            <span style={{ fontSize: 11, color: '#777' }}>Your previous {messages.length - 1} message(s) are saved below. Scroll up to review.</span>
          </div>
        </div>
      )}
        ...
        {loading && (
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <div style={{ padding: '12px 16px', borderRadius: '4px 16px 16px 16px', background: 'rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              {retryCount > 0 && <div style={{ fontSize: 10, color: '#888' }}>Retrying... ({retryCount}/2)</div>}
            </div>
          </div>
        )}
        <div ref={endRef} />
...
export default function App() {
  ...
  return (
    <div style={S.page}>
      ...
      {screen === 'chat'       && <ChatScreen subject={subject} exam={exam} planId={activePlanId} onEnd={() => setScreen('selector')} />}
